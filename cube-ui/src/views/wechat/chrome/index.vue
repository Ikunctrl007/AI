  ...
  methods: {
      ...
      this.enabledAIs.forEach((ai) => {
        if (ai.name === "腾讯元宝T1") {
          ...
          if (ai.selectedCapabilities.includes("deep_thinking")) {
            this.userInfoReq.roles =
              this.userInfoReq.roles + "yb-hunyuan-sdsk,";
          }
          if (ai.selectedCapabilities.includes("web_search")) {
            this.userInfoReq.roles =
              this.userInfoReq.roles + "yb-hunyuan-lwss,";
          }
        }
        if (ai.name === "腾讯元宝DS") {
          ...
          if (ai.selectedCapabilities.includes("deep_thinking")) {
            this.userInfoReq.roles =
              this.userInfoReq.roles + "yb-deepseek-sdsk,";
          }
          if (ai.selectedCapabilities.includes("web_search")) {
            this.userInfoReq.roles =
              this.userInfoReq.roles + "yb-deepseek-lwss,";
          }
        }
        if (ai.name === "AI搜索@元器") {
          this.userInfoReq.roles =
            this.userInfoReq.roles + "cube-trubos-agent,";
        }
        if (ai.name === "数智化助手@元器") {
          this.userInfoReq.roles =
            this.userInfoReq.roles + "cube-turbos-large-agent,";
        }
    ...
    loadHistoryItem(item) {
      try {
        ...
        // 恢复AI选择配置
        console.log(historyData.aiList);
        ...
        this.historyDrawerVisible = false;
        console.log("用户的会话ID", this.userInfoReq);
      } catch (error) {
        console.error("加载历史记录失败:", error);
        this.$message.error("加载历史记录失败");
      }
